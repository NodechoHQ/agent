...
if [ -n "$(command -v timeout)" ]; then
    timeout -s SIGKILL 30 wget -q -o /dev/null -O /etc/nodecho/agent.log -T 25 --post-data "$data_post" --no-check-certificate "https://api.nodecho.com/metrics"
else
    wget -q -o /dev/null -O /etc/nodecho/agent.log -T 25 --post-data "$data_post" --no-check-certificate "https://api.nodecho.com/metrics"
    wget_pid=$!
    wget_counter=0
    wget_timeout=30
